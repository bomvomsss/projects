  ...
  return (
    <div className='bk-card' onClick={() => navigate(`/post/${post.id}`)}>
      ...
      <div className='bk-stats'>
        <span>조회 {formatNum(post.views ?? 0)}</span>
...
function CollectionPage({
  ...
  const groupPosts = selectedGroup
    ? posts.filter((p) => selectedGroup.postIds.includes(p.id))
    : [];
  const addablePosts = bookmarkedPosts.filter(
    (p) => !selectedGroup?.postIds.includes(p.id),
  );
    ...
    <div>
      ...
      <div className='cg-tab-header'>
        <div className='cg-tabs'>
          <button
            className={`cg-tab ${tab === "bookmarks" ? "active" : ""}`}
            onClick={() => {
              setTab("bookmarks");
          ...
          >
            전체 북마크{" "}
            <span className='cg-tab-count'>{bookmarkedPosts.length}</span>
          </button>
          <button
            className={`cg-tab ${tab === "groups" ? "active" : ""}`}
            onClick={() => {
              setTab("groups");
              setSelectedGroupId(null);
            }}
          >
            컬렉션{" "}
            <span className='cg-tab-count'>{collectionGroups.length}</span>
          </button>
        </div>
          ...
          {showNewGroupInput ? (
            <div className='cg-new-input-row'>
              <input
                ...
                autoFocus
              />
              <button className='btn-black' onClick={handleCreateGroup}>
                만들기
              </button>
              <button
          ...
          {collectionGroups.length === 0 ? (
            ...
            <div className='cg-grid'>
              {collectionGroups.map((group) => {
                const cover = posts.find((p) =>
                  group.postIds.includes(p.id),
                )?.coverUrl;
                return (
                  <div
                    key={group.id}
                  ...
                  >
                    ...
                    <div className='cg-card-info'>
                      ...
                      <span className='cg-card-count'>
                        작품 {group.postIds.length}개
                      </span>
                    </div>
                    <div
            ...
            >
              ...
              >
                ...
                {addablePosts.length === 0 ? (
                  <p className='cg-add-modal-empty'>
                    추가할 수 있는 북마크가 없습니다.
                  </p>
                ) : (
                  <ul className='cg-add-list'>
                    {addablePosts.map((post) => (
                      <li key={post.id} className='cg-add-item'>
                        ...
                        <button
                          className='btn-black cg-add-btn-sm'
                          onClick={() => {
                            addPostToGroup(selectedGroupId, post.id);
                          }}
                        >
                          추가
                        </button>
